...
for i in range(len(x_vals)):
	x_vals[i]=x_vals[i]/np.sum(x_vals[i])
...
Alpha=tf.Variable(tf.random_uniform(shape=[n0],minval=0.0,maxval=2*np.pi))

W1=tf.stack([tf.cos(Alpha),tf.sin(Alpha)],1)
#W1=tf.reshape(W1,(n0,n1))
...
metadata = dict(title='Radial Visualization', artist='Radviz',
                comment='Radviz')
...
with writer.saving(fig, "Radviz.mp4", 100):
	for i in range(200):
#def animate(i):
		for j in range(10):
		...
		axes.scatter(V[:,0],V[:,1],color='red')
		#print(V[0].shape)
		...
		xt=np.cos(t)
		yt=np.sin(t)
		axes.plot(xt,yt,color='blue')
			
		axes.set_title("Radial Visualization")
