...
def circleradviz(X,a):
	...
	for i in range(n):
		Y[i]=lineradviz(X[i],anchor,order)
	return Y
...
for i in range(m):
	...
	plt.text(0.5*(np.cos(t1)+np.cos(t2)),0.5*(np.sin(t1)+np.sin(t2)),str(pm[i]),color="black")

#draw unit circle
t = np.linspace(0,2*np.pi, 100)
plt.plot(np.cos(t),np.sin(t),color="blue",alpha=0.95)
